...
// GET /api/rankings?kind=team|debater&circuit=pf|cx  -> the Glicko-2 table, strongest
// first. The two circuits are separate tables: a college policy rating and a Public
// Forum one are not comparable, so they are never shown in one list.
//
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const rows = await loadRatings(db, stored);
    // when the table last changed, so the page can say how fresh it is
    const { data: stamp } = await db.from("ratings").select("updated_at").eq("kind", stored)
    ...
    return NextResponse.json({ kind, circuit, count: table.length, updatedAt: stamp?.updated_at ?? null, rankings: table });
  } catch (e: any) {
