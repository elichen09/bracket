...
export function mountCaselist(ctx) {
  ...
  function loadIndex(slug) {
    ...
    const p = (async () => {
      ...
      }).then((r) => r.json()).then((j) => { ix.ranks = j.ranks || {}; search(); }).catch(() => { /* unranked, not broken */ });
      paintBar();
      search();
      return ix;
    })().catch((e) => {
  ...
  function body(card) {
    ...
    const p = (async () => {
      ...
      if (!html) {
        ...
        if (!r.ok) { const j = await r.json().catch(() => ({})); throw new Error(j.error || 'the card would not open'); }
        html = await r.text();
