...
function toHtml(elems) {
  ...
  elems.forEach((e) => {
    ...
    if (e.bd) css.push('border:1.5pt solid #000000;padding:8pt 10pt');
...
function renderDoc() {
  ...
  if (S.docHtml === null) {
    ...
    host.innerHTML = elems.length
      ...
      : '<div class="docempty">What you send appears here, exactly as it will paste — ' +
        'and you can type in it.</div>';
    return;
  }
...
function withPrefix(head, speech) {
  const out = clone(head);
  const runs = out.runs || [];
  const first = runs.find((r) => (r.t || '').trim());
  if (!first) return out;
  const text = runsText(out);
  if (text.indexOf(speech + '---') === 0) return out;      // already done
  first.t = speech + '---' + first.t.replace(/^\s*/, '');
  return out;
}
...
function formatElements(src, o) {
  const out = [];
  const tally = { blocks: 0, kept: 0, dropped: 0, blanks: 0 };
  const title = o.title ? String(o.title).trim() : '';
  if (title) out.push({ k: 'p', h: 'H1', al: 'center', bd: 1, runs: [{ t: title, b: true, fs: 16 }] });
  ...
  (src || []).forEach((e) => {
    ...
    if (isBlockHead(e)) {
      ...
      n = 0;
      tally.blocks++;
      out.push(o.prefix && o.speech ? withPrefix(e, o.speech) : e);
      return;
    }
...
function openFormat() {
  ...
  $('#card').innerHTML =
    ...
    opt('fmtPrefix', 'Put the speech on each heading', '“AT: Heg good” becomes “' + s.fmtSpeech + '---AT: Heg good”') +
