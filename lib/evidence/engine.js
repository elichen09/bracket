...
async function sendParsed(block, argIndex, from) {
  ...
  if (same >= 0) return mergeInto(same, item, block, from);
 ...
 * other cards are.
 */
function mergeInto(at, item, block, from) {
  const x = S.send[at];
  const have = new Set((x.parts || []).map((q) => q.title));
  const fresh = item.parts.filter((q) => !have.has(q.title));
  if (!fresh.length) { toast('Copied — that card is already in ' + (x.title || 'the send list')); return; }
  ...
  toast('Copied' + (from ? ' ' + from : '') + ' — added to ' + (x.title || 'its block') + ' (' + x.parts.length + ' cards)');
  if (CL && CL.on) return;
  $('#q').value = '';
  runSearch();
}
