...
/** The highlight colour most of a document's highlighted words wear. */
function commonestHighlight(elems) {
  const n = {};
  elems.forEach((e) => (e.runs || []).forEach((r) => { if (r.bg) n[r.bg] = (n[r.bg] || 0) + (r.t || '').length; }));
  let best = '', most = 0;
...
function formatElements(src, o) {
  ...
  // cards; the speech should read as one. On copies — the elements may be the
  // send list's own.
  const hl = o.hl === 'most' ? commonestHighlight(out) : o.hl;
  if (hl) {
    for (let i = 0; i < out.length; i++) {
      const e = out[i];
      if (!(e.runs || []).some((r) => r.bg && r.bg !== hl)) continue;
      const c = clone(e);
      c.runs.forEach((r) => { if (r.bg) r.bg = hl; });
      out[i] = c;
...
function openFormat() {
  ...
  $('#card').innerHTML =
    ...
    (S.cut.length
      ...
      : '') +
    '<div class="set"><label>Highlighting<span class="sub">Cards cut in several colours, made one</span></label><select data-f="fmtHl">' +
