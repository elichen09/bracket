...
function renderResults() {
  ...
  const rowHtml = (ri, i) => {
    ...
    let out = '<div class="row' + (isSel ? ' sel' : '') + '" data-ri="' + ri + '" style="--i:' + i + '">' +
  ...
  // Position in the list, which is how long a row waits before it arrives.
  // Capped, so the twentieth row is not still turning up half a second later.
  let seat = 0;
  const step = () => Math.min(seat++, 16);
  ...
  S.groups.forEach((g) => {
    ...
    html += '<div class="grp' + (gSel ? ' sel' : '') + (open ? '' : ' closed') +
              '" data-grp="' + esc(g.key) + '" style="--i:' + step() + '">' +
            ...
            '</div>';
    if (open) g.rows.forEach((ri) => { html += rowHtml(ri, step()); });
  });

  box.innerHTML = html;
  // The list only arrives when it is a different list. Moving the selection
  // redraws the same rows, and rows that re-animate under every arrow key are
  // worse than rows that never move.
  const shape = S.groups.map((g) => g.key + ':' + (groupOpen(g.key) ? g.rows.join('.') : '')).join('|');
  if (shape !== lastShape) {
    lastShape = shape;
