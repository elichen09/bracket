...
/**
 * Two things a run cannot say, both written the way Google Docs writes them
 * itself, because Docs' own export is the one HTML dialect its importer is
 * certain to read back:
 *
 *   box  a one-cell table. Paragraph borders are a Docs feature but a
 *        fragile paste; a bordered table is how every cover page in every
 *        block file is actually made, and it survives.
 *   brk  <hr style="page-break-before:always">, which is the exact markup
 *        Docs emits for a page break when you download a document as HTML.
 */
const BOX_CELL = 'border:2.25pt solid #000000;padding:10pt 12pt;text-align:center';
...
function toHtml(elems) {
  ...
  elems.forEach((e) => {
    ...
    if (e.k === 'brk') { closeList(); out.push('<hr style="page-break-before:always">'); return; }
    if (e.k === 'box') {
      closeList();
      out.push('<table style="border-collapse:collapse;width:100%"><tbody><tr>' +
        '<td style="' + BOX_CELL + '"><p style="text-align:center;margin:0">' + inner + '</p></td>' +
        '</tr></tbody></table>');
      return;
    }
    if (e.k === 'li') {
...
const copyRich = (elems) => copyMarkup(toHtml(elems), toText(elems));
...
async function copySendDoc() {
  ...
  if (S.docHtml !== null) {
    ...
    try {
      await copyMarkup('<meta charset="utf-8">' + html, text);
...
function formatElements(src, o) {
  ...
  if (title) {
    out.push({ k: 'box', runs: [{ t: title, b: true, fs: 26 }] });
...
function openFormat() {
  ...
  $('#card').innerHTML =
    ...
    opt('fmtPages', 'A page for each block', 'The cover gets one of its own') +
    '</div><div class="out">' +
...
export function boot(el) {
  ...
  formatElements, countAnalytics, docTitle, openFormat, runFormat
  };
