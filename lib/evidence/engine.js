...
function toHtml(elems) {
  ...
  elems.forEach((e) => {
    ...
    if (e.bd) css.push('border:3px solid #333333;padding:6pt 10pt');
...
function flatten(paper) {
  if (!paper) return '';
  return [...paper.querySelectorAll('.sheet')].map((s) => s.innerHTML).join(SHEET_BREAK);
}

/** The paper: every sheet, editable or not. */
function paperHtml(html, editable) {
  const sheets = sheetsFrom(html).map((inner) => '<div class="sheet">' + (inner || '<p><br></p>') + '</div>').join('');
  return '<div class="paper"' + (editable ? ' contenteditable="true" spellcheck="false"' : '') + '>' + sheets + '</div>';
}
...
function onDocInput(ev) {
  const paper = ev.target && ev.target.closest ? ev.target.closest('.paper[contenteditable]') : null;
  if (!paper) return;
  const first = S.docHtml === null;
  paper.querySelectorAll('.sheet.blank').forEach((el) => el.classList.remove('blank'));
  S.docHtml = flatten(paper);
  S.docIds = S.send.map((c) => c.id);
  saveDoc();
