...
function pocketKeyOf(e) { return e.c1 || UNFILED; }

/** The hat below it, written as a path, which is also its fold key. */
function groupKeyOf(e) {
  return [e.c1, e.c2].filter(Boolean).join(' › ') || UNFILED;
}
...
function buildTree() {
  ...
  S.results.forEach((r, ri) => {
    ...
    if (!e.c2) { p.kids.push({ k: 'block', ri: ri }); return; }
    ...
    let h = p.hats[hk];
    if (!h) { h = p.hats[hk] = { key: hk, label: e.c2, rows: [], cards: 0 }; p.kids.push({ k: 'hat', hat: h }); }
    h.rows.push(ri);
    h.cards += (e.n || 0);
  });
  return pockets;
...
function renderResults() {
  ...
  const groupLine = (f, i) => {
    ...
    return '<div class="node lv' + f.lvl + (i === S.sel ? ' sel' : '') + (open ? ' open' : ' shut') +
             '" data-grp="' + esc(f.gk) + '" style="--i:' + seat(i) + '">' +
...
function publishDoc(now) {
  ...
  const go = () => {
    ...
    const payload = { by: roomName || OWNER_NAME || 'Partner', at: Date.now(), blocks };
...
function paintRoom() {
  ...
  if (!dot || !label) return;
  const flows = roomPeers.filter((p) => p.kind === 'flow');
  dot.className = 'dot' + (ROOM ? (flows.length ? ' live' : ' waiting') : '');
  label.textContent = ROOM ? (flows.length ? `Sharing · ${ROOM.code}` : `Room ${ROOM.code}`) : 'Room';
  const st = $('#room-st');
  if (st) {
    st.textContent = !ROOM ? 'Not sharing.'
      : flows.length ? `Sharing with ${flows.map((p) => p.name).join(', ')} — they see every change as you make it.`
...
function joinRoom(code, name) {
  ...
  ROOM = joinFlow(code, roomName, {
    ...
    onEvent: (type) => { if (type === 'ask') publishDoc(true); },
    onPeers: (peers) => {
  ...
  paintRoom();
  toast('Sharing the send doc in room ' + code);
}
function leaveRoom() {
  if (ROOM) ROOM.leave();
  ROOM = null; roomPeers = [];
  paintRoom();
  toast('Stopped sharing the send doc');
}
...
function openRoom() {
  ...
  $('#card').className = 'card';
  $('#card').innerHTML =
    '<h2>Share the send doc</h2><div class="in">' +
    '<p class="hint">One of you builds the send doc here; the other flows it. Join the same room code ' +
    'as the flow, and your partner\'s <b>Flow</b> gets a <b>Send doc</b> tab that follows every change you ' +
    'make — blocks and tags, ready to drop down a column. Your own Flow tabs see it too, room or not. ' +
    'Nothing is stored anywhere.</p>' +
    ...
    '</div><div class="out">' +
    '<button class="btn key" id="room-join" data-act="roomJoin">Share in this room</button>' +
    '<button class="btn" id="room-leave" data-act="roomLeave">Stop sharing</button>' +
...
export function boot(el, opts = {}) {
  ...
  BUS = openBus(OWNER, async (m) => {
    if (m.kind !== 'send-changed' || !root) return;
    try {
