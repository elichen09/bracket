...
async function copyRich(elems) {
  const html = toHtml(elems);
  const text = toText(elems);
  try {
...
function renderDoc() {
  ...
  const elems = read ? buildReadElements() : buildSendElements();
  const label = $('#doclabel');
  if (label) label.textContent = read ? 'Read doc' : 'Send doc';
  if (!elems.length) {
    host.innerHTML = '<div class="docempty">' +
      (read ? 'The speech document builds itself as you send cards.'
            : 'What you send appears here, exactly as it will paste.') + '</div>';
    return;
  }
  // toHtml leads with the charset meta the clipboard needs; on a page it is
  // just an empty node sitting where the first paragraph should be.
  host.innerHTML = '<div class="page">' + toHtml(elems).replace(/^<meta[^>]*>/, '') + '</div>';
}
...
async function start() {
  loadSettings();
  try {
    const [index, meta, send] = await Promise.all([
      DB.get('kv', 'index'), DB.get('kv', 'meta'), DB.get('kv', 'send')
    ]);
    S.index = prepare(index || []);
    S.meta = meta || null;
...
export function boot(el) {
  ...
  renderCase, openCase, copyCaseDoc, renderDoc, buildSendElements
  };
