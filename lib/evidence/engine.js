...
const DEFAULTS = {
  ...
  fmtTidy: true,
  fmtPages: true,
  fmtHl: 'most'        // every highlight one colour: 'most' (the commonest), a hex, or '' to leave them
};
...
const S = {
  ...
  docIds: [],
  /**
   * Analytics Format cut, with where each one was — its block, and the
   * argument it came after — so they can be put back into the document as
   * it is now, not as it was. Kept with the document; a rebuild from the
   * send list brings them back anyway, so it clears this.
   */
  cut: [],
  /**
...
function saveDoc() {
  ...
  docTimer = setTimeout(() => {
    ...
    else DB.set('kv', 'doc', { html: S.docHtml, ids: S.docIds, cut: S.cut }).catch(() => {});
  }, 400);
}
...
function resetDoc() {
  S.cut = [];            // the rebuild has every analytic in it again
...
async function sendParsed(block, argIndex, from) {
  ...
  // The copy is a convenience; the send is the point. A browser that will not
  // let this page at the clipboard must not stop the card reaching the send
  // list — and the shared send doc — so a refusal is said, not obeyed.
  let copied = true;
  try { await copyRich(entryElems(item)); }
  catch (e) { copied = false; }
  const said = (copied ? 'Copied' : 'Sent (the browser would not let it be copied)') + (from ? ' ' + from : '');

  // A block already in the list takes the card in, rather than a second
  // "AT: Animals" turning up under the first.
  const same = item.blockId ? S.send.findIndex((c) => c.blockId === item.blockId) : -1;
  if (same >= 0) return mergeInto(same, item, block, said);
  ...
  renderSide();
  landed = -1;
  // Said while the row it came from is still on screen, for the motion layer
  // to carry across to the list.
  ...
  toast(said + (copied ? ' — press Ctrl+V in your Send doc' : ''));
  // The search stays: the next card often comes from the same one.
}

/**
 * More cards from a block already in the send list: they join it, each in
 * the block's own order, and the send document gets them where the block's
 * other cards are.
 */
function mergeInto(at, item, block, said) {
  const x = S.send[at];
  const have = new Set((x.parts || []).map((q) => q.title));
  const fresh = item.parts.filter((q) => !have.has(q.title));
  if (!fresh.length) { toast(said + ' — that card is already in ' + (x.title || 'the send list')); return; }
  const order = (t) => (block.args || []).findIndex((a) => (a.title || '(untitled)') === t);
  fresh.forEach((q) => {
    const k = order(q.title);
    let to = x.parts.length;
    if (k >= 0) { const j = x.parts.findIndex((r) => order(r.title) > k); if (j >= 0) to = j; }
    x.parts.splice(to, 0, q);
  });
  placeParts(x, fresh);
  persistSend();
  landed = at;
  renderSide();
  landed = -1;
  root.dispatchEvent(new CustomEvent('evi:sent', { detail: { title: x.title } }));
  if (BUS) BUS.post({ kind: 'sent', title: x.title, trigger: x.trigger || '', tags: fresh.map((q) => q.title) });
  toast(said + ' — added to ' + (x.title || 'its block') + ' (' + x.parts.length + ' cards)');
}

/**
 * New cards of a block that is already in a written send document go in
 * after the block's card before them — found by its tag, whatever number
 * it carries now — or before the one after, or at the end. A shared send
 * doc is being typed in by a partner, so there they join the end instead.
 */
function placeParts(x, fresh) {
  const elemsOf = (list) => list.reduce((a, q) => a.concat((q.elems || []).map(clone)), []);
  if (SHARE && SHARE.ready) {
    if (SHARE.placed.has(x.id)) appendShared(pageHtml(elemsOf(fresh).concat([{ k: 'p', h: 'P', runs: [] }])));
    return;
  }
  if (S.docHtml === null || !S.docIds.includes(x.id)) return;      // it is drawn from the list, or joins whole
  const els = elementsFromHtml(S.docHtml);
  const key = (t) => String(t || '').replace(LEAD_NUM, '').toLowerCase().replace(/[^a-z0-9]+/g, '');
  const find = (title) => els.findIndex((e) => isTagline(e) && key(runsText(e)) === key(title));
  const endOf = (i) => { for (let j = i + 1; j < els.length; j++) if (isTagline(els[j]) || isBlockHead(els[j])) return j; return els.length; };
  fresh.forEach((q) => {
    const k = x.parts.indexOf(q);
    let at = -1;
    // after the nearest card before it that is in the document (new ones go in order, so that may be one just placed)
    for (let j = k - 1; j >= 0 && at < 0; j--) { const i = find(x.parts[j].title); if (i >= 0) at = endOf(i); }
    for (let j = k + 1; j < x.parts.length && at < 0; j++) { const i = find(x.parts[j].title); if (i >= 0) at = i; }
    if (at < 0) at = els.length;
    els.splice(at, 0, ...(q.elems || []).map(clone));
  });
  S.docHtml = pageHtml(els);
  saveDoc();
}
...
/**
 * A number a tag already starts with, however it was written: "1.", "1)",
 * "1:", "1 -", "1---", "1 —". The dashes count, because a file numbered
 * "1---Pascal's Wager" is numbered.
 */
const LEAD_NUM = /^\s*(\d{1,3})\s*(?:[.):\]]|-{1,3}|[–—])\s*/;
const leadNumber = (e) => { const m = runsText(e).match(LEAD_NUM); return m ? Number(m[1]) : null; };

function withNumber(head, n) {
  const out = clone(head);
  const runs = out.runs || [];
  // Whatever number it had comes off first, across runs if need be — the
  // "1" can be one run and the "---" the next — so it is never "1. 1---".
  const m = runsText(out).match(LEAD_NUM);
  let cut = m ? m[0].length : (runsText(out).match(/^\s*/) || [''])[0].length;
  for (const r of runs) {
    if (!cut) break;
    const t = r.t || '';
    const take = Math.min(cut, t.length);
    r.t = t.slice(take);
    cut -= take;
  }
  const first = runs.find((r) => (r.t || '').trim()) || runs[0];
  if (!first) return out;
  first.t = n + '. ' + first.t;
  return out;
}
 ...
 */
/**
 * A heading that already says its speech, wherever it says it: "AT: Maiese---2AC",
 * "2NC---Theory", "T---2NC---Framing". Any speech, between dashes or at either end.
 */
...
/**
 * A grey highlight: a colour with next to no hue in it (Word's Gray-25% and
 * Gray-50%, #d9d9d9 and the like). Grey is what a rehighlighted card wears
 * for the words its first cutter read and this one does not — a different
 * thing from a highlight, so it is never made one colour with the rest.
 */
function isGreyHighlight(hex) {
...
/** The highlight colour most of a document's highlighted words wear — grey aside. */
function commonestHighlight(elems) {
  const n = {};
  elems.forEach((e) => (e.runs || []).forEach((r) => { if (r.bg && !isGreyHighlight(r.bg)) n[r.bg] = (n[r.bg] || 0) + (r.t || '').length; }));
  let best = '', most = 0;
  Object.keys(n).forEach((k) => { if (n[k] > most) { most = n[k]; best = k; } });
  return best;
}
...
function formatElements(src, o) {
  ...
  let arg = null;
  let firstBlock = true;
  let kept = [];               // this block's arguments, held until it ends
  let cutHere = [];            // analytics cut from it, and how many kept came before each
  let block = '';              // the block heading they are under, as it will read
  const cut = [];
  ...
  const flush = () => {
    ...
    if (o.drop && !card) {
      tally.dropped++;
      cutHere.push({ at: kept.length, elems: [arg.head].concat(arg.body) });
      arg = null;
      return;
    }
    tally.kept++;
  ...
  /**
   * A block's arguments, out. Numbered only if they are not already: a block
   * whose tags already read 1, 2, 3 in order is left as its author wrote it;
   * one with a number missing, out of order, or none at all is numbered.
   */
  const emit = () => {
    const inOrder = kept.length > 0 && kept.every((a, i) => leadNumber(a.head) === i + 1);
    const heads = [];
    kept.forEach((a, i) => {
      const h = o.renumber && !inOrder ? withNumber(a.head, i + 1) : a.head;
      heads.push(h);
      out.push(h);
      a.body.forEach((e) => out.push(e));
    });
    cutHere.forEach((c) => cut.push({ block, after: c.at > 0 ? runsText(heads[c.at - 1]) : null, elems: c.elems }));
    kept = [];
    cutHere = [];
  };
  ...
  (src || []).forEach((e) => {
    ...
    if (isBlockHead(e)) {
      ...
      firstBlock = false;
      const head = o.tag && o.speech ? withTag(e, o.speech) : e;
      block = runsText(head);
      out.push(head);
      return;
    }
  ...
  // Every highlight one colour. A card cut in three colours reads as three
  // cards; the speech should read as one. Grey stays grey: on a rehighlighted
  // card it marks what is not read, not a second colour of what is. On
  // copies — the elements may be the send list's own.
  const hl = o.hl === 'most' ? commonestHighlight(out) : o.hl;
  if (hl) {
    const recolour = (r) => r.bg && r.bg !== hl && !isGreyHighlight(r.bg);
    for (let i = 0; i < out.length; i++) {
      const e = out[i];
      if (!(e.runs || []).some(recolour)) continue;
      const c = clone(e);
      c.runs.forEach((r) => { if (recolour(r)) r.bg = hl; });
      out[i] = c;
      tally.recoloured = (tally.recoloured || 0) + 1;
    }
  }
...
function openFormat() {
  ...
  $('#card').innerHTML =
    ...
    (S.cut.length
      ? '<div class="set cutback"><label>' + S.cut.length + ' analytic' + (S.cut.length === 1 ? '' : 's') + ' cut before' +
        '<span class="sub">Each goes back where it was — after the argument it followed — into the document as it is now</span></label>' +
        '<button class="btn" data-act="analyticsBack">Put ' + (S.cut.length === 1 ? 'it' : 'them') + ' back</button></div>'
      : '') +
    '<div class="set"><label>Highlighting<span class="sub">Cards cut in several colours, made one (grey stays grey)</span></label><select data-f="fmtHl">' +
      [['most', 'All one colour — the commonest'], ['#ffff00', 'All yellow'], ['#00ffff', 'All cyan'], ['#00ff00', 'All green'], ['', 'Leave each as it is']]
        .map(([v, l]) => '<option value="' + v + '"' + ((s.fmtHl === undefined ? 'most' : s.fmtHl) === v ? ' selected' : '') + '>' + l + '</option>').join('') +
      '</select></div>' +
...
/**
 * The analytics Format cut, back where they were in the document as it is
 * now: after the argument each one followed (found by its words, whatever
 * number it carries now), or first in its block, or — if its block has gone
 * — at the end. Then the block is renumbered if Format numbers them.
 */
function putAnalyticsBack() {
  if (!S.cut.length) return;
  const els = S.docHtml === null ? buildSendElements() : elementsFromHtml(S.docHtml);
  const key = (t) => String(t || '').replace(LEAD_NUM, '').toLowerCase().replace(/[^a-z0-9]+/g, '');
  const endOf = (from) => { for (let i = from + 1; i < els.length; i++) if (isBlockHead(els[i])) return i; return els.length; };
  let last = null;           // where the previous one went, so two in a row keep their order
  S.cut.forEach((c) => {
    const bi = c.block ? els.findIndex((e) => isBlockHead(e) && key(runsText(e)) === key(c.block)) : -1;
    let at;
    if (last && last.block === c.block && last.after === c.after) at = last.end;
    else if (c.block && bi < 0) at = els.length;
    else {
      const stop = endOf(bi);
      if (c.after) {
        let ai = -1;
        for (let i = bi + 1; i < stop; i++) if (isTagline(els[i]) && key(runsText(els[i])) === key(c.after)) { ai = i; break; }
        at = stop;
        if (ai >= 0) for (let i = ai + 1; i < stop; i++) if (isTagline(els[i])) { at = i; break; }
      } else {
        at = stop;
        for (let i = bi + 1; i < stop; i++) if (isTagline(els[i])) { at = i; break; }
      }
    }
    const back = c.elems.map(clone);
    els.splice(at, 0, ...back);
    last = { block: c.block, after: c.after, end: at + back.length };
  });
  const n = S.cut.length;
  const out = S.settings.fmtRenumber ? formatElements(els, { renumber: true }) : els;
  S.cut = [];
  ...
  closeModal();
  renderDoc();
  toast(n + ' analytic' + (n === 1 ? '' : 's') + ' put back');
}
...
function runFormat() {
  ...
  const out = formatElements(src, {
    ...
    hl: s.fmtHl === undefined ? 'most' : s.fmtHl,
    tag: !!s.fmtTag,
    tidy: !!s.fmtTidy,
    pages: !!s.fmtPages,
  });
  ...
  if (t.dropped) bits.push(t.dropped + ' analytic' + (t.dropped === 1 ? '' : 's') + ' cut (Format puts them back)');
  if (t.blanks) bits.push(t.blanks + ' blank line' + (t.blanks === 1 ? '' : 's') + ' gone');
  if (t.recoloured) bits.push('highlighting made one colour');
...
async function onClick(ev) {
  ...
  if (act) {
    ...
    if (a === 'analyticsBack') putAnalyticsBack();
    if (a === 'clearSend') {
...
async function start() {
  ...
  try {
    ...
    if (doc && typeof doc.html === 'string') { S.docHtml = doc.html; S.docIds = doc.ids || []; S.cut = Array.isArray(doc.cut) ? doc.cut : []; }
  S.docText = '';
  } catch (e) {
...
function onSharedChange(txn) {
  ...
  if (txn.origin !== Y_ORIGIN && (txn.changed.has(SHARE.list) || txn.changed.has(SHARE.items))) {
    pullSend();
    renderSide();
    // and on to this tab's flows: a partner's reorder or cut changes the send
    // doc as much as their typing does
    publishDoc();
  }
...
/* A pop-out of the send doc (Peek.tsx) — the send or read doc in a window of
   its own — says it is open every little while; while it is, the doc goes to
   it as HTML with every change. Nobody asking, nothing is built. */
let peekAt = 0;
let peekWant = 'send';
function postPeek() {
  if (!BUS || Date.now() - peekAt > 90000) return;
  let html;
  try { html = pageHtml(peekWant === 'read' ? buildReadElements() : sendDocElements()); } catch (e) { return; }
  BUS.post({ kind: 'evidence-doc', want: peekWant, html, at: Date.now() });
}
...
/** The note that brings this tab back into its room after a page change (lib/roomResume.ts). */
let keeping = null;
const resumeKey = () => scoped('evidence.resume', OWNER);

function joinRoom(code, name, back) {
  ...
  openShare(code);
  if (keeping) keeping();
  keeping = keepRoom(resumeKey(), code);
  paintRoom();
  toast(back ? 'Back in room ' + code : 'In room ' + code + ' — one send doc, for both of you');
}
function leaveRoom() {
  closeShare();
  if (ROOM) ROOM.leave();
...
export function boot(el, opts = {}) {
  ...
  BUS = openBus(OWNER, async (m) => {
    if (!root) return;
    if (m.kind === 'evidence-ask') { peekAt = Date.now(); peekWant = m.want === 'read' ? 'read' : 'send'; postPeek(); return; }
    ...
    try {
      ...
      // and into the room's send doc, when there is one, as a card sent here would
      pushSend();
      landed = S.send.length - 1;
      renderSide();
      landed = -1;
      toast('From ' + (m.from || 'Flow') + ': ' + (m.title || 'a card') + ' is in the send list');
  ...
  sendParsed, putAnalyticsBack, pageHtml
  };

  start();
  // A link with a room in it — the one Flow hands a partner — goes straight in.
  // A room from a link — or the one this tab was in a moment ago, before a page
  // change (opening Split screen, a reload): straight back into it.
  const back = opts.room ? null : roomToResume(resumeKey());
  if (opts.room || back) {
    let name = OWNER_NAME;
    try { name = localStorage.getItem(scoped('evidence.me', OWNER)) || OWNER_NAME; } catch (e) { /* private browsing */ }
    joinRoom(opts.room || back, name, !!back);
  }
  paintRoom();
  ...
  return () => {
    ...
    clearTimeout(docTimer);
    closeShare();
    // a page change, not a decision to leave: the note stays for the next page
    if (keeping) { keeping(); keeping = null; }
