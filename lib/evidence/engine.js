 ...
 *
 *   bd   the rule round a cover title. It is a border on the heading itself,
 *        because that is what a cover page is -- a Heading 1 in a box, not a
 *        table with a heading in it.
 *   brk  a page break. Docs throws these away on paste, whatever they are
 *        written as, so this one is for the page on screen; `forPaste` takes
 *        it out again before anything reaches the clipboard, since what Docs
 *        does keep is the horizontal rule, and a stray line through the
...
function toHtml(elems) {
  ...
  elems.forEach((e) => {
    ...
    if (e.bd) css.push('border:2.25pt solid #000000;padding:6pt 10pt');
...
function renderDoc() {
  ...
  if (read) {
    ...
    host.innerHTML = elems.length
      ? '<div class="page">' + pageHtml(elems) + '</div>'
  ...
  if (S.docHtml === null) {
    ...
    host.innerHTML = elems.length
      ? '<div class="page" contenteditable="true" spellcheck="false">' + pageHtml(elems) + '</div>'
      : '<div class="page blank" contenteditable="true" spellcheck="false"><p><br></p></div>';
    paginate();
    return;
  }
  ...
  host.innerHTML =
    ...
    '<div class="page" contenteditable="true" spellcheck="false">' + S.docHtml + '</div>';
  paginate();
}

/* --------------------------------------------------------------- paging
   US Letter at 96dpi, with Docs' inch margins. The panel draws the document
   at these real sizes and zooms the whole page to fit, so a line breaks
   where it will break for the other team.

   A page break has to end the sheet it is on: white to the foot of the page,
   the gap between sheets, then the next page. How much white that is depends
   on where the text happened to reach, so it is measured rather than
   guessed, and re-measured when the document changes. */

const SHEET_H = 1056;             // 11in
const MARGIN = 96;                // 1in
const TEXT_H = SHEET_H - MARGIN * 2;
const GUTTER = 40;                // the grey between two sheets
const RULE = '#c6c1b5';

function paginate() {
  const page = $('#docbody .page');
  if (!page) return;
  const breaks = [...page.querySelectorAll('hr.brk')];
  if (!breaks.length) return;

  const top = page.clientTop + MARGIN;       // border + margin = where the text starts
  let sheet = 0;                             // text-top of the sheet being filled

  breaks.forEach((hr) => {
    hr.style.height = '0px';                 // measure the text alone first
    const used = (hr.offsetTop - top) - sheet;
    const rest = Math.max(0, (TEXT_H - (used % TEXT_H)) % TEXT_H) + MARGIN;
    hr.style.height = (rest + GUTTER + MARGIN) + 'px';
    hr.style.background =
      'linear-gradient(to bottom,' +
      ' #ffffff 0 ' + rest + 'px,' +
      ' ' + RULE + ' ' + rest + 'px ' + (rest + 1) + 'px,' +
      ' var(--surface-2) ' + (rest + 1) + 'px ' + (rest + GUTTER) + 'px,' +
      ' ' + RULE + ' ' + (rest + GUTTER) + 'px ' + (rest + GUTTER + 1) + 'px,' +
      ' #ffffff ' + (rest + GUTTER + 1) + 'px 100%)';
    sheet = (hr.offsetTop - top) + rest + GUTTER + MARGIN;
  });
}
...
function onDocInput(ev) {
  const page = ev.target && ev.target.closest ? ev.target.closest('.page[contenteditable]') : null;
  if (!page) return;
  const first = S.docHtml === null;
  page.classList.remove('blank');
  S.docHtml = page.innerHTML;
  S.docIds = S.send.map((c) => c.id);
  saveDoc();
  clearTimeout(pageTimer);
  pageTimer = setTimeout(paginate, 350);
  ...
  if (first) {
    ...
    page.parentNode.insertBefore(note, page);
  }
}
...
async function copySendDoc() {
  ...
  if (S.docHtml !== null) {
    const page = $('#docbody .page');
    const html = page ? page.innerHTML : S.docHtml;
    const text = page ? page.innerText : html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
...
export function boot(el) {
  ...
  formatElements, countAnalytics, docTitle, openFormat, runFormat, paginate
  };
