 ...
 *
 * `argIndex` picks one argument out of the block, or a list of them picks
 * several; leave it null for the whole block. A block with no arguments sends whatever sits under its heading.
 */

const uid = (p) => (p || 'id') + '_' + Math.random().toString(36).slice(2, 10);

export function makeSendItem(block, argIndex, includeHead) {
  const pick = Array.isArray(argIndex) ? argIndex.filter((i) => i >= 0)
    : (argIndex !== null && argIndex !== undefined && argIndex >= 0 ? [argIndex] : null);
  const single = !!(pick && pick.length);
  const args = single ? pick.map((i) => (block.args || [])[i]).filter(Boolean) : (block.args || []);
