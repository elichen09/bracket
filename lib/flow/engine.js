...
export function boot(root, opts = {}) {
  ...
  function chrome() {
    $('#tabs').innerHTML = S.sheets.map((s, i) =>
      ...
      `<button type="button" role="tab" class="${s.side}" data-i="${i}" aria-selected="${i === A.cur}" title="Click again to rename · right-click for more">${esc(s.name)}` +
      `${new RegExp('\\b' + s.side + '\\b', 'i').test(s.name) ? '' : `<i>${s.side}</i>`}</button>`).join('') +
  ...
  on($('#tabs'), 'contextmenu', (e) => {
    ...
    const items = [
      ...
      '-',
      ['Delete this sheet', '', () => { A.cur = i; render(); deleteSheet(); }, 'danger'],
    ];
