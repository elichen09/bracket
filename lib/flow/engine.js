...
export function boot(root, opts = {}) {
  ...
  function openMenu(e, p) {
    ...
    const items = [
      ...
      ['Mark turn', '', () => tag('TURN')],
      [stopAt(A.sel) ? 'Take out of round vision' : 'Add to round vision', K + 'B', () => toggleStop()],
      ['Find evidence that answers this', K + '/', () => answerFromEvidence()],
  ...
  on(document, 'keydown', (e) => {
    ...
    // Round vision: ⌥1–9 goes straight to a stop, ⌘] and ⌘[ walk them in order.
  ...
  async function renameSheet() {
    const sheet = sheetOf();
    const tab = $(`#tabs button[data-i="${A.cur}"]`);
    const name = await ask('Name this sheet', sheet.name, tab || A.flow);
    if (name === null) return;
    snap();
  ...
  const hitHtml = (h, i) =>
    ...
    `<span class="acts"><button class="ins" type="button" data-i="${i}" data-send="0">Flow it</button>` +
    `<button class="ins go" type="button" data-i="${i}" data-send="1" title="Into the flow, and into Evidence's send list">Flow + send</button></span></article>`;
  ...
  async function runCards() {
    ...
    if (!q) { box.innerHTML = `<p class="small">${INDEX.length} blocks in your library. Type to search, or select an argument on the flow and press ${K}/.</p>`; return; }
  ...
  async function flowIt(hit, alsoSend, target) {
    ...
    if (!t || !rows()[t.r] || !rows()[t.r].c) return toast('Pick a cell on the flow first', true);
    if (editing) commit();
    snap();
    const row = rows()[t.r];
    ...
    if (ok) { BUS.post({ kind: 'send-changed', title: hit.title }); toast('Flowed, and in your send list', true); }
  ...
  on($('#inbox'), 'click', (e) => {
    ...
    if (all) {
      ...
      if (!s || !A.sel) return toast('Pick a cell on the flow first', true);
      if (editing) commit();
  ...
  /* ================================================================
     ...
     cell, and lights it. ⌥1–9 goes straight to a stop; a presentation
  ...
  async function toggleStop() {
    ...
    toast(`Stop ${n} — ${name.trim() || guess}${n <= 9 ? ' · ⌥' + n : ''}`);
  }
  ...
  function goStop(i) {
    const v = S.vision[i];
    if (!v) return toast(S.vision.length ? `There is no stop ${i + 1}` : `No round vision yet — ${K}B marks a cell`);
    const at = resolveStop(v);
    visionAt = i;
    paintSpeak(); paintVision();
    if (!at) return toast(`“${v.name}” is not on the flow any more`, true);
    if (editing) commit();
  ...
  function stepStop(d) {
    if (!S.vision.length) return toast(`No round vision yet — ${K}B marks a cell`);
  ...
  function paintVision() {
    ...
    if (!list) return;
    if (!S.vision.length) {
      list.innerHTML = `<li class="none"><span class="small">Nothing yet. Select a cell you mean to go for and press ${K}B.</span></li>`;
    } else {
      list.innerHTML = S.vision.map((v, i) => {
        ...
        return `<li class="vs${i === visionAt ? ' now' : ''}${at ? '' : ' gone'}" draggable="true" data-i="${i}" style="--i:${Math.min(i, 10)}">` +
          ...
          `${i < 9 ? `<kbd>⌥${i + 1}</kbd>` : '<span></span>'}<button type="button" class="x" data-x="${i}" aria-label="Remove">×</button></li>`;
      }).join('');
    }
  ...
  function speak(onNow) {
    if (onNow && !S.vision.length) return toast(`No round vision yet — ${K}B marks a cell`);
    speaking = onNow;
   ...
   * with / searches this account's Evidence library instead, and ⌘/ opens it
   * already searching for whatever answers the argument you are on.
   */
  const CMDS = [
    ['Flow', 'Undo', K + 'Z', undo],
    ['Flow', 'Redo', K + '⇧Z', redo],
    ['Flow', 'Mark dropped', K + 'D', () => tag('DROPPED')],
    ['Flow', 'Mark extended', K + 'E', () => tag('EXT')],
    ['Flow', 'Mark turn', '', () => tag('TURN')],
    ['Flow', 'Insert row below', '', () => A.sel && insertRow(A.sel.r + 1, A.sel.c, true)],
    ['Flow', 'Insert a heading', '', () => A.sel && insertHeading(A.sel.r)],
    ['Flow', 'Clear the selected cells', '⌫', () => A.sel && clearCells(A.anchor ? rangeCells() : [A.sel])],
    ['Flow', 'Delete row', K + '⌫', () => A.sel && deleteRows(A.anchor ? rangeCells().map((x) => x.r) : [A.sel.r])],
    ['Flow', 'Delete cell, shift the column up', '⇧⌫', () => A.sel && deleteCellUp(A.sel.r, A.sel.c)],
    ['Flow', 'Respond in the next column', '⇥', () => A.sel && setSel(A.sel.r, Math.min(LAST, A.sel.c + 1), true)],
    ['Round vision', 'Add this cell to round vision', K + 'B', () => toggleStop()],
    ['Round vision', 'Speak through round vision', 'PgDn', () => speak(true)],
    ['Round vision', 'Next stop', K + ']', () => stepStop(1)],
    ['Round vision', 'Previous stop', K + '[', () => stepStop(-1)],
    ...
    ['Evidence', 'Find evidence that answers this', K + '/', () => answerFromEvidence()],
    ['Evidence', 'Search the library', '/', () => openPal('/')],
    ['Evidence', 'Open Evidence', '', () => window.open('/tools/evidence', 'break-evidence')],
    ['Sheets', 'New Pro sheet', '', () => addSheet('pro')],
    ['Sheets', 'New Con sheet', '', () => addSheet('con')],
    ['Sheets', 'Rename this sheet', '', renameSheet],
    ['Sheets', 'Delete this sheet', '', deleteSheet],
    ['Sheets', 'Copy this sheet as text', '', () => copyText(false)],
    ['Sheets', 'Copy every sheet as text', '', () => copyText(true)],
    ['Clock', 'Start / pause the clock', K + '.', toggleTimer],
    ['Clock', 'Flip who speaks first', '', flipFirst],
    ...PREP_CHOICES.map((s) => ['Clock', `Prep time — ${clock(s)} each`, '', () => setPrep(s)]),
    ['View', 'Split screen', K + '\\', () => setSplit(split === 'side' ? null : 'side')],
    ['View', 'Stack screens', '', () => setSplit(split === 'stack' ? null : 'stack')],
    ['View', 'Switch screen', K + "'", swapScreens],
    ['View', 'Widen the column I am in', '', () => $('#v-focus').click()],
    ['View', 'Tighter rows', '', () => $('#v-compact').click()],
    ['View', 'Notes, cards and round vision', K + 'J', () => toggleDrawer()],
  ...
  function places() {
    const out = [];
    S.vision.forEach((v, i) => out.push(['Round vision', `Go to · ${v.name}`, i < 9 ? '⌥' + (i + 1) : '', () => goStop(i),
      (() => { const at = resolveStop(v); return at ? `${at.sheet.name} · ${COLS[v.col].key}` : 'not on the flow'; })()]));
    S.sheets.forEach((s, i) => out.push(['Sheets', `Go to · ${s.name}`, '', () => {
  ...
  async function listPal() {
    ...
    $('#pal-foot').innerHTML = pal.mode === 'ev'
      ? `<span><kbd>↵</kbd> flow it</span><span><kbd>${K}↵</kbd> flow it and send it</span><span><kbd>esc</kbd> close</span>`
      : `<span><kbd>↑↓</kbd> move</span><span><kbd>↵</kbd> run</span><span><kbd>/</kbd> search your evidence</span><span><kbd>esc</kbd> close</span>`;
    ...
    if (pal.mode === 'ev') {
      ...
      pal.items = hits.map((h) => ({ group: 'Evidence', label: h.title, sub: `/${h.trigger}${h.path ? ' · ' + h.path : ''}`, hit: h }));
    ...
    if (!q) {
      ...
      const order = ['Round vision', 'From Evidence', 'Evidence', 'Flow', 'Sheets', 'Clock', 'View', 'Together', 'Round'];
  ...
  function palKeys(e) {
    ...
    if (e.key === 'Enter') { e.preventDefault(); runPal(pal.idx, mod(e)); }
  }
  let palTimer = null;
  on($('#pal-q'), 'input', () => { pal.idx = 0; clearTimeout(palTimer); palTimer = setTimeout(listPal, $('#pal-q').value.startsWith('/') ? 90 : 0); });
  on($('#pal-list'), 'click', (e) => { const li = e.target.closest('.it'); if (li) runPal(+li.dataset.j, e.metaKey || e.ctrlKey); });
