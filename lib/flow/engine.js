...
import { library, find, sendToEvidence, sendBlockToEvidence, blockTags, readBins, setBinUse } from './cards';
import { caselistClient, ORDER as CL_ORDER, SHORT as CL_SHORT, familyOf, shortOf, roundOf, norm as clNorm } from '../caselist/client';
...
import { keepRoom, roomToResume, forgetRoom } from '../roomResume';
...
export function boot(root, opts = {}) {
    ...
    sheet.rows.forEach((row, ri) => {
      ...
      if (row.h !== undefined && !row.c) {
        ...
        html += `<div class="rh${b}" data-r="${ri}"><span class="k">[${String(hi).padStart(2, '0')}]</span>` +
          `<span class="hn">${esc(row.h)}</span><em>${esc(row.t || '')}</em>` +
          `<button type="button" class="del" data-r="${ri}" title="Delete heading" aria-label="Delete heading">×</button></div>`;
        return;
      }
  ...
  function chrome() {
    $('#tabs').innerHTML = '<div class="tscroll">' + S.sheets.map((s, i) =>
      ...
      `<span class="tx" data-del="${i}" title="Delete this flow" aria-label="Delete ${esc(s.name)}">×</span></button>`).join('') + '</div>' +
      '<div class="tadd"><button type="button" class="plus" data-add="pro">+ Pro</button><button type="button" class="plus" data-add="con">+ Con</button>' +
      '<button type="button" class="plus sdp" data-sdopen title="Their case off SpeechDrop, tags down a column">⇩ SpeechDrop</button></div>';
    fitTabs();
  ...
  /** A wide foot scrolls sideways, fades at the edge that has more, and keeps this flow's tab in sight. */
  const FOOT = root.querySelector('.bottom');
  function fitTabs() {
    if (!FOOT) return;
    const on = $('#tabs [aria-selected="true"]');
    if (on) {
      const a = on.getBoundingClientRect(), f = FOOT.getBoundingClientRect();
      if (a.left < f.left + 36) FOOT.scrollLeft -= f.left + 36 - a.left;
      else if (a.right > f.right - 36) FOOT.scrollLeft += a.right - (f.right - 36);
    }
    edgeFoot();
  }
  function edgeFoot() {
    if (!FOOT) return;
    const over = FOOT.scrollWidth > FOOT.clientWidth + 1;
    FOOT.classList.toggle('fl', over && FOOT.scrollLeft > 2);
    FOOT.classList.toggle('fr', over && FOOT.scrollLeft + FOOT.clientWidth < FOOT.scrollWidth - 2);
  }
  on(FOOT, 'scroll', edgeFoot, { passive: true });
  on(FOOT, 'wheel', (e) => {
    if (FOOT.scrollWidth <= FOOT.clientWidth || Math.abs(e.deltaX) > Math.abs(e.deltaY)) return;
    e.preventDefault();
    FOOT.scrollLeft += e.deltaY;
  }, { passive: false });
  on(window, 'resize', edgeFoot);
  ...
  function deleteRows(rs, label) {
    rs = [...new Set(rs)].filter((r) => rows()[r]).sort((a, b) => b - a);
    if (!rs.length) return;
    snap();
    const heads = rs.filter((r) => !rows()[r].c).length;
    if (!label && heads === rs.length) label = heads > 1 ? `Deleted ${heads} headings` : 'Heading deleted';
    rs.forEach((r) => A.grid.querySelectorAll(`[data-r="${r}"].c,[data-r="${r}"].g,[data-r="${r}"].rh`).forEach((x) => x.classList.add('dying')));
    setTimeout(() => {
      rs.forEach((r) => rows().splice(r, 1));
      if (!rows().some((x) => x.c)) rows().push({ id: uid('r'), c: blankC() });
  ...
  PANES.forEach((p) => {
    on(p.grid, 'pointerdown', (e) => {
      ...
      if (p === A && A.anchor && rangeCells().length > 1 && rangeCells().some((x) => x.r === r && x.c === c)) {
        const cells = rangeCells();
        press = { kind: 'block', p, r, c, x: e.clientX, y: e.clientY, cells,
          r0: Math.min(A.anchor.r, A.sel.r), r1: Math.max(A.anchor.r, A.sel.r),
          c0: Math.min(A.anchor.c, A.sel.c), c1: Math.max(A.anchor.c, A.sel.c) };
        return;
      }
  ...
  on(window, 'pointermove', (e) => {
    if (press && !drag && Math.hypot(e.clientX - press.x, e.clientY - press.y) > 5) {
      ...
      if (press.kind === 'block' && !press.cells.some(({ r, c }) => (rows(press.p)[r].c[c] || '').trim())) return;
      if (editing) commit();
      drag = press;
      ...
      if (drag.kind === 'cell') {
        ...
        const first = drag.cells.map(({ r, c }) => rows(drag.p)[r].c[c]).find((t) => t && t.trim()) || '';
        ghost.innerHTML = esc(first) + `<small>${drag.cells.length} cells · up or down</small>`;
        drag.cells.forEach(({ r, c }) => { const x = cellEl(r, c, drag.p); if (x) x.classList.add('dragsrc'); });
      } else {
    ...
    if (drag) {
      ...
      if (drag.kind === 'cell') {
        ...
        $$('.c.dropb').forEach((x) => x.classList.remove('dropb'));
        drag.by = blockShift(drag, rowUnder(e.clientY, drag.p));
        blockLanding(drag, drag.by).forEach(({ r, c }) => { const x = cellEl(r, c, drag.p); if (x) x.classList.add('dropb'); });
        autoScroll(e, drag.p);
      } else {
  ...
  on(window, 'pointerup', (e) => {
    if (drag) {
      ...
      $$('.dropb').forEach((x) => x.classList.remove('dropb'));
      if (drag.kind === 'block' && drag.by) moveBlock(drag, drag.by);
    ...
    if (press && (press.kind === 'cell' || press.kind === 'block')) { const { r, c } = press; press = null; setSel(r, c, true); return; }
    press = null;
  });

  /**
   * A block of cells moves up or down its own columns. Heading rows are
   * stepped over, the cells it lands on slide into the gap it left, and
   * rows are added below if it goes past the end.
   */
  const cellRowsOf = (p) => rows(p).map((row, i) => (row.c ? i : -1)).filter((i) => i >= 0);
  /** The row under the pointer, or the last one if it is below them all. */
  function rowUnder(y, p) {
    const els = [...p.grid.querySelectorAll('.r > .g')];   // .r is display: contents — its gutter has the size
    for (const el of els) { const b = el.getBoundingClientRect(); if (y < b.bottom) return +el.dataset.r; }
    const last = els[els.length - 1];
    if (!last) return 0;
    const b = last.getBoundingClientRect();
    // past the end: one more row for every row's height below it
    return +last.dataset.r + Math.max(0, Math.floor((y - b.bottom) / Math.max(20, b.height)) + 1);
  }
  /** How many cell rows the block moves, from where it was picked up to the row under the pointer. */
  function blockShift(d, under) {
    const cr = cellRowsOf(d.p);
    const at = (r) => { const i = cr.indexOf(r); if (i >= 0) return i; if (r >= rows(d.p).length) return cr.length - 1 + (r - rows(d.p).length + 1); const n = cr.findIndex((x) => x > r); return n < 0 ? cr.length - 1 : n; };
    let by = at(under) - at(d.r);
    const top = at(d.r0);
    if (top + by < 0) by = -top;
    return by;
  }
  /** The cells the block would cover, as far as the rows exist yet. */
  function blockLanding(d, by) {
    if (!by) return [];
    const cr = cellRowsOf(d.p);
    const out = [];
    const lo = cr.indexOf(d.r0), hi = cr.indexOf(d.r1);
    for (let i = lo; i <= hi; i++) {
      const r = cr[i + by];
      if (r === undefined) continue;
      for (let c = d.c0; c <= d.c1; c++) out.push({ r, c });
    }
    return out;
  }
  function moveBlock(d, by) {
    const p = d.p, rs = rows(p);
    snap();
    let cr = cellRowsOf(p);
    const lo = cr.indexOf(d.r0), hi = cr.indexOf(d.r1);
    while (hi + by >= cr.length) { rs.push({ id: uid('r'), c: blankC() }); cr = cellRowsOf(p); }
    const s0 = Math.min(lo, lo + by), s1 = Math.max(hi, hi + by);
    for (let c = d.c0; c <= d.c1; c++) {
      const seg = [];
      for (let i = s0; i <= s1; i++) seg.push(rs[cr[i]].c[c]);
      const block = seg.slice(lo - s0, hi - s0 + 1);
      const rest = seg.slice(0, lo - s0).concat(seg.slice(hi - s0 + 1));
      const next = by > 0 ? rest.concat(block) : block.concat(rest);
      for (let i = s0; i <= s1; i++) rs[cr[i]].c[c] = next[i - s0];
    }
    A = p; paintActive();
    A.anchor = { r: cr[lo + by], c: d.c0 }; A.sel = { r: cr[hi + by], c: d.c1 };
    render(); A.flow.focus({ preventScroll: true });
    blockLanding(d, by).forEach(({ r, c }) => flash(cellEl(r, c)));
    save(); pushRows(sheetOf(p));
    const n = Math.abs(by);
    toast(`Moved ${d.cells.length} cells ${by > 0 ? 'down' : 'up'} ${n} row${n === 1 ? '' : 's'}`, true);
  }
  ...
  function openMenu(e, p) {
    ...
    if (rh) {
      ...
      return showMenu(e, [
        ['Rename heading', '', () => renameHeading(r)],
        ['Insert row above', '', () => insertRow(r, 0, true)],
        ['Insert row below', '', () => insertRow(r + 1, 0, true)], '-',
        ['Delete heading', '', () => deleteRows([r]), 'danger'],
      ]);
    }
    const el = e.target.closest('.c');
    if (!el) return;
  ...
  on(window, 'keydown', (e) => {
    ...
    if (document.querySelector('.fpick')) return;                      // Which round? has the keys
  ...
  async function renameHeading(r) {
    ...
    const h = await ask('Heading', row.t ? `${row.h} — ${row.t}` : row.h, el || A.flow, 'Empty it to delete the heading');
    if (h === null) return;
    if (!h.trim()) return deleteRows([r]);
    snap();
   ...
   */
  function placeDown(t, lines, stop = () => false) {
    const rs = rows();
    const c = t.c;
    let r = t.r;
    const born = [];
    // A row takes the next line when this column is empty there — beside the
    // case's own lines, the way a flow is written on paper — unless it is
    // another argument's to answer: a heading, the next contention, a row
    // another block is going beside. Then a row is made for the line instead.
    const free = (n) => !!(rs[n] && rs[n].c && !(rs[n].c[c] || '').trim()) && !startsArgument(rs, n, c) && !stop(n);
    lines.forEach((ln, i) => {
      if (i === 0 && rs[r] && rs[r].c && !rs[r].c[c].trim()) { rs[r].c[c] = ln; return; }
      const n = r + 1;
      if (!free(n)) { rs.splice(n, 0, { id: uid('r'), c: blankC() }); born.push(n); }
      r = n;
      rs[r].c[c] = ln;
    });
    return { r, born };
  }
  /** Where a new argument begins, in the column to the left: a contention, an advantage, an observation. */
  const CONTENTION = /^\s*(?:(?:our\s+)?(?:first|second|third|fourth|fifth|sixth|next|final|last)\s+(?:contention|advantage|observation)|(?:contention|advantage|observation)\s*(?:\d+|one|two|three|four|five|six|[ivx]+)\b|(?:adv|ob|c)\s*\d+\b)/i;
  function startsArgument(rs, n, c) {
    const row = rs[n];
    if (!row) return false;
    if (row.h !== undefined && !row.c) return true;
    return c > 0 && !!row.c && CONTENTION.test(row.c[c - 1] || '');
  }
  ...
  // Which send doc to show: the one picked with the switch, if one was — and
  // otherwise the newer of the two, so a copy that has stopped moving (a tab
  // out of the room, one a step behind) never hides the one that has not.
  let docPicked = false;
  const currentDoc = () => {
    if (docPicked && DOCS[docShown]) return DOCS[docShown];
    const r = DOCS.room, t = DOCS.tab;
    return r && t ? (t.at > r.at ? t : r) : r || t;
  };
  ...
  function paintSendDoc() {
    ...
    let html = '<div class="sdhead">' +
      ...
      `<span class="small">${DOCS.room && DOCS.tab && !both ? 'The shared' : doc.from === 'room' ? esc(doc.by) + '’s' : 'Your'} send doc · ${when} · ${doc.blocks.length} block${doc.blocks.length === 1 ? '' : 's'}</span>` +
      ...
      // the whole speech at once: every block's header and tags, in order, one box each
      `<button type="button" class="ins go sdall" data-sd-all title="Every block of this speech — each header and every tag, in order — down the column you are in">Flow all ↓</button>` +
      '</div>';
    let section = null;
  ...
  on($('#sdoc'), 'click', (e) => {
    ...
    if (show) { docShown = show.dataset.sdShow; docPicked = true; paintSendDoc(); return; }
    const doc = currentDoc();
    ...
    if (e.target.closest('[data-sd-all]') && doc) {
      flowBlocks(doc.blocks.map((b) => [b.head, ...b.tags.map((t) => t.tag)].filter(Boolean)));
      return;
    }
  ...
  /* What an argument or a block is about, for matching one to the other:
     "AT: Innovation---2NC", "A2 innovation", "C1---Innovation", "Contention 1:
     Innovation" and "Innovation" are all "innovation". The answering words
     go, the speech a block is for goes, the contention's number goes. */
  const SPEECH_PART = /^(?:\d?\s*(?:ac|nc|ar|nr|1ar|2ar|1nr|2nr|1nc|2nc|1ac|2ac|pc|cc|ff|sum|summ|summary|reb|rebuttal|case|fl|frontline|ext|extension)|pro|con|aff|neg|us|them)$/i;
  const LABEL_PART = /^(?:c|con|contention|adv|advantage|sp|sub-?point|sub|ob|observation|pt|point|arg)\s*\d+[a-z]?$|^\d+[a-z]?$|^[a-z]$/i;
  const topicWords = (text, isBlock) => {
    let t = String(text || '').split('\n')[0].trim();
    if (isBlock) t = t.replace(/^\s*(?:at|a2|a\/t|answers?\s+to|ans\s+to|vs\.?|responses?\s+to|frontlines?|fl)\b\s*[:\-–—.]*\s*/i, '');
    t = t.replace(/^\s*(?:our\s+)?(?:(?:first|second|third|fourth|fifth|sixth|next|final|last)\s+(?:contention|advantage|observation)|(?:c|contention|adv(?:antage)?|sub-?point|sp|ob(?:servation)?)\s*(?:\d+[a-z]?|one|two|three|four|five|six|[ivx]+))\s*(?:[:.)\-–—]+|\s+is\b|\s+—)\s*/i, '')
      .replace(/^\s*(?:\d+|[a-z])\s*[.)]\s+/i, '');
    const parts = t.split(/\s*[-–—]{2,}\s*|\s+[-–—|]\s+/).map((x) => x.trim()).filter((x) => x && !SPEECH_PART.test(x) && !LABEL_PART.test(x));
    return parts.join(' ').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, ' ').replace(/^(?:the|a|an)\s+/, '').trim().split(' ').filter(Boolean);
  };
  /** How well a block's topic names an argument: 2 the same words, 1 most of them, 0 not it. */
  const sameTopic = (blockW, argW) => {
    if (!blockW.length || !argW.length) return 0;
    const a = blockW.join(' '), b = argW.join(' ');
    if (a === b) return 2;
    const [small, big] = blockW.length <= argW.length ? [blockW, argW] : [argW, blockW];
    const set = new Set(big);
    return small.every((w) => set.has(w)) && small.length * 2 >= big.length ? 1 : 0;
  };

  /**
   * A whole speech's blocks, each beside the argument it answers.
   *
   * By name first: a block called "AT: Innovation---2NC" (or "AT: innovation")
   * goes beside the argument written "C1---Innovation" (or just "Innovation")
   * in the column to the left, or under a flow heading of that name — wherever
   * it is on the sheet. A block no argument is named for takes the next
   * argument no block has claimed, down from the row you are on (the first
   * starts on that row), so a speech in case order still lands in order. A
   * block with more lines than the room beside its argument pushes the rest
   * down rather than writing into the next one's; blocks left over when the
   * arguments run out carry on down the column.
   */
  function flowBlocks(groups) {
    groups = groups.filter((g) => g.length);
    if (!A.sel || !rows()[A.sel.r] || !rows()[A.sel.r].c) return toast('Pick a cell on the flow first — the one beside the first argument');
    if (!groups.length) return;
    if (editing) commit();
    snap();
    const rs = rows(), c = A.sel.c, from = A.sel.r;
    const argAt = (r) => c > 0 && rs[r] && rs[r].c && !!(rs[r].c[c - 1] || '').trim();
    // Rows are known by id: every block placed can push the rows under it down.
    const idx = (id) => rs.findIndex((x) => x.id === id);
    const startId = rs[from].id;

    // what each block is claiming, before anything moves: the best-named argument
    // (or heading) not already claimed, a same-words match beating a near one
    const claimed = new Map();          // group index -> { id, heading }
    const taken = new Set();
    const candidates = rs.map((row) => ({
      id: row.id,
      heading: row.h !== undefined && !row.c,
      words: row.h !== undefined && !row.c ? topicWords(row.h, false) : row.c && c > 0 ? topicWords(row.c[c - 1], false) : [],
    })).filter((x) => x.words.length);
    for (const want of [2, 1]) {
      groups.forEach((lines, gi) => {
        if (claimed.has(gi)) return;
        const bw = topicWords(lines[0], true);
        const hit = candidates.find((x) => !taken.has(x.id) && sameTopic(bw, x.words) === want);
        if (hit) { claimed.set(gi, hit); taken.add(hit.id); }
      });
    }

    const born = [];
    let byName = 0;
    const ends = [];                   // the first and last row of each block, by id
    // blocks found by name go where their argument is; the rest keep their own
    // place, down from the row you picked, one unclaimed argument after another
    let seqId = null;
    const nextFree = (n) => {
      while (n < rs.length && (!argAt(n) || taken.has(rs[n].id))) n++;
      if (n >= rs.length) { rs.push({ id: uid('r'), c: blankC() }); n = rs.length - 1; born.push(n); }
      return n;
    };
    groups.forEach((lines, gi) => {
      let at;
      const mine = claimed.get(gi);
      if (mine) {
        at = idx(mine.id);
        if (mine.heading) {
          // under a heading: its first row of arguments, made if there is none
          let n = at + 1;
          while (n < rs.length && !rs[n].c) n++;
          if (n >= rs.length || (rs[n].h !== undefined && !rs[n].c)) { rs.splice(n, 0, { id: uid('r'), c: blankC() }); born.push(n); }
          at = n;
        }
        byName++;
      } else {
        // the row you picked, unless a block named for it has it already
        at = seqId === null ? (taken.has(startId) ? nextFree(idx(startId) + 1) : idx(startId)) : nextFree(idx(seqId) + 1);
        if (argAt(at)) taken.add(rs[at].id);
      }
      // down beside the case's lines — but never into a row another block has claimed
      const out = placeDown({ r: at, c }, lines, (n) => taken.has(rs[n].id) && rs[n].id !== rs[at].id);
      born.push(...out.born);
      if (!mine) seqId = rs[out.r].id;
      ends.push(rs[at].id, rs[out.r].id);
    });
    // where it all went, now that every block has pushed the rows under it down
    const spots = ends.map(idx).filter((i) => i >= 0);
    const firstAt = Math.min(...spots), last = Math.max(...spots);
    render(born);
    setSel(last, c, false);
    popIn(Math.min(from, firstAt), last, c);
    save(); pushRows(sheetOf());
    const n = groups.length;
    toast(`${n} block${n === 1 ? '' : 's'} down ${colsOf()[c].key}` +
      (byName ? ` — ${byName === n ? (n === 1 ? 'found' : 'each found') : byName + ' found'} by name${byName < n ? ', the rest in order' : ''}` : ', each beside its argument'), true);
  }
  ...
  const pal = { items: [], idx: 0, mode: 'cmd', target: null, source: '', pick: null, src: 'lib' };
  /* '/' searches your Evidence library — or this season's caselist wikis, by tag,
     from the same weekly index Evidence's Caselist search uses (lib/caselist/client.js). */
  const PALSRC_KEY = scoped('flow.palsrc', OWNER);
  const CLWIKIS_KEY = scoped('caselist.wikis', OWNER);        // the same wikis Evidence has switched on
  try { if (localStorage.getItem(PALSRC_KEY) === 'cl') pal.src = 'cl'; } catch { /* private browsing */ }
  const clWikis = () => { try { return new Set(JSON.parse(localStorage.getItem(CLWIKIS_KEY) || '["hspf"]')); } catch { return new Set(['hspf']); } };
  let CLC = null;
  const caselist = () => {
    if (!CLC) {
      CLC = caselistClient(OWNER, scoped);
      let soon = null;
      offs.push(CLC.on((what, detail) => {
        if (what === 'error') { toast(String(detail)); return; }
        if ($('#scrim').hidden || pal.mode !== 'ev' || pal.src !== 'cl' || pal.pick) return;
        clearTimeout(soon); soon = setTimeout(() => { const keep = pal.idx; listPal().then(() => { pal.idx = Math.min(keep, Math.max(0, pal.items.length - 1)); moveHl(true); }); }, 80);
      }));
    }
    return CLC;
  };
  function setPalSrc(src) {
    pal.src = src;
    try { localStorage.setItem(PALSRC_KEY, src); } catch { /* private browsing */ }
    pal.idx = 0;
    listPal();
    $('#pal-q').focus();
  }
  ...
  async function paintPalBins(show) {
    const box = $('#pal-bins');
    if (!show || pal.pick || $('#scrim').hidden) { box.hidden = true; return; }
    const sw = `<span class="psrc" role="group" aria-label="Search in"><button type="button" data-psrc="lib" aria-pressed="${pal.src === 'lib'}">Library</button>` +
      `<button type="button" data-psrc="cl" aria-pressed="${pal.src === 'cl'}">Caselist</button></span>`;
    if (pal.src === 'cl') {
      const C = caselist(), chosen = clWikis();
      const metas = C.st.metas || [];
      const chips = CL_ORDER.map((fam) => {
        const m = metas.find((x) => familyOf(x.wiki) === fam);
        const ld = m && C.st.loading.get(m.wiki);
        const n = !m ? (C.st.metas ? '—' : '') : ld ? (ld.total ? Math.round((ld.got / ld.total) * 100) + '%' : '…') : C.st.idx.has(m.wiki) ? (m.cards >= 10000 ? Math.round(m.cards / 1000) + 'k' : m.cards) : '';
        return `<button type="button" class="pbin${chosen.has(fam) ? ' on' : ''}" data-pwiki="${fam}" title="${chosen.has(fam) ? 'Searched — click to leave it out' : 'Left out — click to search it'}"><i></i>${CL_SHORT[fam]}<em>${n}</em></button>`;
      }).join('');
      box.innerHTML = sw + '<span class="pb-l">Wikis</span>' + chips;
      box.hidden = false;
      return;
    }
    const { bins, counts } = await readBins(OWNER);
    if (pal.src !== 'lib') return;
    ...
    box.innerHTML = sw + (bins.list.length ? '<span class="pb-l">Bins this round</span>' + bins.list.map((b) => chip(b.id, b.name, b.on)).join('') + (counts[''] ? chip('', 'Unsorted', bins.loose) : '') : '');
    box.hidden = false;
  }
  ...
  async function listPal() {
    ...
    $('#pal-foot').innerHTML = pal.mode === 'ev'
      ? (pal.src === 'cl'
        ? `<span><kbd>${kb('enter')}</kbd> flow + send</span><span><kbd>${kb('shift+enter')}</kbd> flow only</span><span><kbd>Tab</kbd> your library</span><span><kbd>${kb('esc')}</kbd> close</span>`
        : `<span><kbd>${kb('enter')}</kbd> open the block's cards</span><span><kbd>Tab</kbd> the caselist</span><span><kbd>${kb('esc')}</kbd> close</span>`)
    ...
    if (pal.mode === 'ev' && pal.src === 'cl') {
      const C = caselist(), chosen = clWikis();
      C.warm(chosen);
      const loading = [...C.st.loading.entries()].filter(([slug]) => chosen.has(familyOf(slug)));
      if (!q) {
        const tags = [...C.st.idx.entries()].filter(([slug]) => chosen.has(familyOf(slug))).reduce((n, [, ix]) => n + ix.cards.length, 0);
        pal.items = [];
        paintPal(`<li class="empty">${loading.length ? 'Getting the caselist index ready…' : tags ? tags.toLocaleString() + ' caselist tags. Type what a tag says.' : 'Type what a tag says.'}</li>`);
        return;
      }
      const { groups } = C.find(q, chosen);
      const items = [];
      for (const g of groups) {
        for (const c of g.cards) {
          if (items.length >= 80) break;
          items.push({ group: 'Caselist', label: c.tag, cl: c,
            sub: `${g.rank ? '#' + g.rank.rank + ' ' : ''}${g.name} · ${shortOf(c.slug)} · ${roundOf(c.path)}${c.rounds > 1 ? ' · ' + c.rounds + ' rounds' : ''}` });
        }
        if (items.length >= 80) break;
      }
      pal.items = items;
      if (!items.length) { paintPal(`<li class="empty">${loading.length ? 'Getting the caselist index ready…' : 'No caselist tag says that.'}</li>`); return; }
      paintPal();
      return;
    }
    if (pal.mode === 'ev') {
  ...
  function paintPal(empty) {
    ...
    pal.items.forEach((it, j) => {
      ...
      html += `<li class="it${j === pal.idx ? ' on' : ''}${it.all ? ' all' : ''}" data-j="${j}" style="--i:${Math.min(j, 14)}">` +
        ...
        `<span class="lb"><span class="lt">${lit(it.label, it.hits)}</span>${it.sub ? `<small>${esc(it.sub)}</small>` : ''}</span>` +
        `${typed && it.group !== 'Evidence' && it.group !== 'Caselist' ? `<span class="gtag">${esc(it.group)}</span>` : ''}` +
  ...
  function runPal(j, withSend) {
    ...
    if (it.cl) return flowCaselist(it.cl, !!withSend);
    closePal();
    if (it.run) it.run();
  }
  /**
   * A caselist card: its tag into the flow, down the column, the way a library
   * tag goes — and, unless Shift is held, the card itself into Evidence's send
   * list, read out of its document the way Evidence reads one.
   */
  async function flowCaselist(card, withSend) {
    const target = pal.target;
    closePal();
    await flowIt({ title: card.tag, text: card.tag }, false, target, [card.tag]);
    if (!withSend) return;
    try {
      const html = await caselist().cardHtml(card);
      const { parseHtml } = await import('../evidence/engine');
      const blk = parseHtml(html, {}).blocks.find((b) => (b.args || []).length);
      if (!blk) throw new Error('that card came back empty');
      const block = { ...blk, id: 'cl_' + card.path + '#' + clNorm(blk.title), pre: [], args: [blk.args[0]] };
      const ok = await sendBlockToEvidence(OWNER, block, 0);
      if (!ok) throw new Error('Evidence would not take it');
      BUS.post({ kind: 'send-changed', title: blk.title });
      toast('Flowed — and the card is in your send doc', true);
    } catch (err) { toast('Flowed — but the card did not reach the send doc: ' + (err && err.message || err)); }
  }

  function palKeys(e) {
    // in '/', Tab switches between your library and the caselist
    if (!pal.pick && pal.mode === 'ev' && e.key === 'Tab') { e.preventDefault(); setPalSrc(pal.src === 'cl' ? 'lib' : 'cl'); return; }
    if (pal.pick) {
  ...
  on($('#pal-bins'), 'click', async (e) => {
    const src = e.target.closest('[data-psrc]');
    if (src) { setPalSrc(src.dataset.psrc); return; }
    const wk = e.target.closest('[data-pwiki]');
    if (wk) {
      const w = clWikis();
      if (w.has(wk.dataset.pwiki)) { if (w.size > 1) w.delete(wk.dataset.pwiki); } else w.add(wk.dataset.pwiki);
      try { localStorage.setItem(CLWIKIS_KEY, JSON.stringify([...w])); } catch { /* private browsing */ }
      await listPal();
      $('#pal-q').focus();
      return;
    }
  ...
  const RESUME_KEY = scoped('flow.resume', OWNER);
  let keeping = null;       // the note that brings this tab back into the room after a page change
  ...
  function paintPeers(p = A) {
    // .pmark, not .peer: .peer is the partner chip in the Share dialog, and its
    // styles (white uppercase type on a pill) would dress the whole cell
    p.grid.querySelectorAll('.c.pmark').forEach((x) => { x.classList.remove('pmark'); x.removeAttribute('data-peer'); x.style.removeProperty('--hue'); });
    const sheet = sheetOf(p);
    peers.forEach((peer) => {
  ...
  function connect(code, back) {
    ...
    if (keeping) keeping();
    keeping = keepRoom(RESUME_KEY, code);
    paintShare(); paintSendDoc();
    toast(back ? 'Back in room ' + code : 'Room ' + code + ' — copy the link for your partner');
  }
  ...
  on($('#share-leave'), 'click', () => {
    ...
    if (keeping) { keeping(); keeping = null; }
    forgetRoom(RESUME_KEY);
  ...
  // a room this tab was in a moment ago — before opening Split screen, a reload —
  // is gone straight back into, rather than left behind by a page change
  const back = !opts.join && !opts.open ? roomToResume(RESUME_KEY) : null;
  if (opts.join) { openShare(); $('#share-in').value = opts.join; }
  else if (back) { $('#share-name').value = meName(); connect(back, true); }
  else if (opts.open) {
    ...
    // Which round? The page asks; these are what it can do about the answer.
    opts.ask({
      current: () => { const s = summarizeGrid(S, Date.now()); return { id: S.id, name: s.name, stats: s.stats, blank: !gridHasWriting(S) }; },
      open: (id) => getRound(OWNER, id).then((rec) => {
        if (!rec || rec.kind !== 'grid') return toast('That round is not in Past flows any more');
        if (rec.id !== S.id) openRound(rec.data, rec.name);
        A.flow.focus({ preventScroll: true });
      }),
      fresh: () => { newRound(); A.flow.focus({ preventScroll: true }); },
      focus: () => A.flow.focus({ preventScroll: true }),
    });
  }
  ...
  return () => {
    ...
    try { localStorage.setItem(scoped(STORE, OWNER), JSON.stringify(S)); } catch { /* private browsing */ }
    // Unmounting is a page change, not a decision to leave: the room is let go,
    // but the note stays, so the next page comes back into it.
    if (keeping) keeping();
    if (live) live.leave();
    document.body.classList.remove('flw-dragging');
  };
