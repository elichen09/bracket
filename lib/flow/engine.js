...
export function boot(root, opts = {}) {
  ...
  /**
   * A whole speech's blocks, each beside the argument it answers. The first
   * goes by the row you are on; each after it by the next argument down the
   * column to the left — the case, for a rebuttal — so "AT: econ" lands on the
   * econ contention and "AT: tech race" on the one under it. A block with more
   * lines than the room beside its argument pushes the rest down rather than
   * writing into the next one's; blocks left over when the arguments run out
   * carry on down the column.
   */
  function flowBlocks(groups) {
    ...
    const born = [];
    let at = from, last = from;
    groups.forEach((lines, i) => {
      if (i > 0) {
        let n = last + 1;
        while (n < rs.length && !argAt(n)) n++;
        at = n < rs.length ? n : last + 1;
        if (at >= rs.length) { rs.push({ id: uid('r'), c: blankC() }); at = rs.length - 1; born.push(at); }
      }
    ...
    popIn(from, last, c);
    save(); pushRows(sheetOf());
    toast(`${groups.length} block${groups.length === 1 ? '' : 's'} down ${colsOf()[c].key}, each beside its argument`, true);
  }
