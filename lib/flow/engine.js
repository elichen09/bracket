...
export function boot(root, opts = {}) {
  ...
  function chrome() {
    $('#tabs').innerHTML = S.sheets.map((s, i) =>
      `<button type="button" role="tab" class="${s.side}" data-i="${i}" aria-selected="${i === A.cur}">${esc(s.name)}<i>${s.side}</i></button>`).join('') +
  ...
  function openMenu(e, p) {
    ...
    const items = [
      ...
      [split ? 'Open this sheet on the other screen' : 'Open in split screen', kb('mod+\\'), () => openBeside(A.cur)], '-',
   ...
   */
  const CMDS = [
    ['Flow', 'Undo', kb('mod+Z'), undo],
    ['Flow', 'Redo', kb('mod+shift+Z'), redo],
    ['Flow', 'Mark dropped', kb('mod+D'), () => tag('DROPPED')],
    ['Flow', 'Mark extended', kb('mod+E'), () => tag('EXT')],
    ...
    ['Flow', 'Clear the selected cells', kb('back'), () => A.sel && clearCells(A.anchor ? rangeCells() : [A.sel])],
    ['Flow', 'Delete row', kb('mod+back'), () => A.sel && deleteRows(A.anchor ? rangeCells().map((x) => x.r) : [A.sel.r])],
    ['Flow', 'Delete cell, shift the column up', kb('shift+back'), () => A.sel && deleteCellUp(A.sel.r, A.sel.c)],
    ['Flow', 'Respond in the next column', kb('tab'), () => A.sel && setSel(A.sel.r, Math.min(LAST, A.sel.c + 1), true)],
  ...
  function places() {
    ...
    COLS.forEach((col, i) => out.push(['Flow', `Flow in · ${col.long}`, kb('mod+shift+' + (i + 1)), () => jumpCol(i), '']));
  ...
  async function listPal() {
    ...
    $('#pal-foot').innerHTML = pal.mode === 'ev'
      ? `<span><kbd>${kb('enter')}</kbd> flow it</span><span><kbd>${kb('mod+enter')}</kbd> flow it and send it</span><span><kbd>${kb('esc')}</kbd> close</span>`
