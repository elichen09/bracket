...
export function boot(root, opts = {}) {
  ...
  /** Many flows: the strip scrolls, fades at the edge it hides, keeps this flow's tab in sight. */
  function fitTabs(left = 0) {
    const sc = $('#tabs .tscroll');
    if (!sc) return;
    sc.scrollLeft = left;
    const on = sc.querySelector('[aria-selected="true"]');
    if (on) {
      const a = on.offsetLeft - sc.offsetLeft, b = a + on.offsetWidth;
      if (a < sc.scrollLeft + 24) sc.scrollLeft = Math.max(0, a - 24);
      else if (b > sc.scrollLeft + sc.clientWidth - 24) sc.scrollLeft = b - sc.clientWidth + 24;
    }
    edgeTabs();
  }
  function edgeTabs() {
    ...
    sc.classList.toggle('fl', over && sc.scrollLeft > 2);
    sc.classList.toggle('fr', over && sc.scrollLeft + sc.clientWidth < sc.scrollWidth - 2);
    $('#tabs').closest('.bottom').classList.toggle('crowded', over);
  }
  on($('#tabs'), 'scroll', edgeTabs, true);
  on($('#tabs'), 'wheel', (e) => {
    const sc = e.target.closest('.tscroll');
    if (!sc || sc.scrollWidth <= sc.clientWidth || Math.abs(e.deltaX) > Math.abs(e.deltaY)) return;
    e.preventDefault();
    sc.scrollLeft += e.deltaY;
  }, { passive: false });
  on(window, 'resize', edgeTabs);
