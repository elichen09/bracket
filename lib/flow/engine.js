...
export function boot(root, opts = {}) {
  ...
  const ACTIONS = [
    ...
    ...[1, 2, 3, 4, 5, 6, 7, 8].map((n) => ['Columns', 'col.' + n, () => `Write in ${COLS[n - 1].long}`, ['mod+shift+' + n], 'any', () => jumpCol(n - 1)]),
  ...
  function paintSendDoc() {
    ...
    let html = '<div class="sdhead">' +
      ...
      `<span class="small">${doc.from === 'room' ? esc(doc.by) + '’s' : 'Your'} send doc · ${when} · ${doc.blocks.length} block${doc.blocks.length === 1 ? '' : 's'}</span></div>`;
    let section = null;
  ...
  function flowLines(lines) {
    ...
    let r = A.sel.r;
    const born = [];
    const blank = () => ({ id: uid('r'), c: Array(8).fill('') });
    lines.forEach((ln) => {
      while (r < rs.length && rs[r].c && rs[r].c[c].trim()) r++;
      if (r >= rs.length || !rs[r].c) { rs.splice(r, 0, blank()); born.push(r); }
      rs[r].c[c] = ln;
      r++;
    });
    while (r < rs.length && !rs[r].c) r++;
    if (r >= rs.length) { rs.push(blank()); born.push(rs.length - 1); }
    render(born);
    setSel(r, c, false);
    save(); pushRows(sheetOf());
    toast(lines.length > 1 ? `${lines.length} lines down ${COLS[c].key}` : 'Into ' + COLS[c].key, true);
  }
  ...
  function places() {
    ...
    COLS.forEach((col, i) => out.push(['Editing', `Write in · ${col.long}`, '@col.' + (i + 1), () => jumpCol(i), '']));
  ...
  function paintPeers(p = A) {
    ...
    peers.forEach((peer) => {
      ...
      box.style.width = Math.min(el.offsetWidth + 1, p.grid.offsetWidth - el.offsetLeft) + 'px';
  ...
  function paintShare() {
    ...
    $('#share-state').textContent = !on ? 'Not shared'
      : peers.length ? `${peers.filter((p) => p.kind !== 'evidence').length + 1} flowing${peers.some((p) => p.kind === 'evidence') ? ' · send doc' : ''}` : 'Waiting for your partner';
    ...
    $('#share-peers').innerHTML = peers.map((p) =>
      `<span class="peer${p.kind === 'evidence' ? ' evi' : ''}" style="--hue:${p.hue}">${esc(p.name)}${p.kind === 'evidence' ? ' · send doc' : ''}</span>`).join('') || '';
    $('#share-evi').hidden = !on;
  }
  function openShare() {
